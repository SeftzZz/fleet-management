            ...
            <!-- <form method="post" title="starter-plan" enctype="multipart/form-data" action="<?php echo base_url('cms/home/insertFormKamarPackage/'.$this->session->userdata('idBusiness').'/') ?>" id="demo-form2" id="demo-form2" class="form-horizontal form-label-left"> -->
              ...
              <div class="item form-group">
                ...
                document.getElementById('locale').addEventListener('change', function() {
                    ...
                    // Kosongkan dropdown eventType
                    eventTypeSelect.innerHTML = '<option value="">--Choose Event--</option>';

                    // Panggil server untuk mendapatkan data berdasarkan bahasa
                    fetch(`<?php echo base_url('cms/home/get_event_types'); ?>?locale=${locale}`)
                        .then(response => response.json())
                        .then(data => {
                            // Tambahkan opsi baru berdasarkan data dari server
                            data.forEach(eventType => {
                                const option = document.createElement('option');
                                option.value = eventType.titleHero;
                                option.textContent = eventType.titleHero;
