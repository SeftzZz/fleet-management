...
class Home extends CI_Controller
{
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($data["user"]) {
                ...
                if($this->session->userdata('level') == '5') {
                    ...
                    redirect("cms/home/viewBooking/".$this->session->userdata('idBusiness')."/");
                } else {
    ...
    {
        $folderName = $this->input->post("folderName"); // Folder yang dipilih atau dibuat
        $kodeUploads = $this->input->post("kodeUploads"); // Folder yang dipilih atau dibuat
        if (empty($folderName)) {
            echo json_encode([
                "status" => "error",
        ...
        if (!empty($_FILES["file"])) {
            $files = $_FILES["file"];
            $idUser = 1; // Ambil idUser dari session jika ada

            // Query untuk mendapatkan path folder dari database
            $this->db->select(["path", "id"]);
            $this->db->from("folders");
            $this->db->where("name", $folderName); // Pastikan nama folder yang dicari
            $query = $this->db->get();

            // Cek apakah folder ada
            if ($query->num_rows() > 0) {
                $folder = $query->row();
                $uploadDir = $folder->path; // Ambil path dari hasil query
                $idDir = $folder->id; // Ambil path dari hasil query
                ...
                foreach ($files["name"] as $key => $file_name) {
                    ...
                    // Pindahkan file dari temporary ke direktori yang ditentukan
                    if (
                        ...
                        $data = [
                            ...
                            "nmUploads"         => $file_name,
                            "fileUploads"       => $file_path,
                            "folder_id"         => $idDir,
                            "created_at_uploads"=> date("Y-m-d H:i:s"),
                        ];

                        // Insert data ke tabel uploads
                ...
                echo json_encode([
                    "status" => "success",
                    "message" => "File berhasil diunggah di " . $folderName,
                ]);
                // Call the Python script to process the file
                // $this->process_file_with_python($file_path, base_url());
            } else {
                echo json_encode([
                    "status" => "error",
    ...
    public function viewRoomAttendant()
    {
        ...
        $data[
            "taskAttendant"
        ] = $this->Housekeeping_m->get_task_attendant_with_detail_by_businessId_and_today(
            $this->session->userdata("idBusiness")
        );
        $data["taskTypes"] = $this->Housekeeping_m->get_task_type(
            $this->session->userdata("idBusiness")
        );
        $data[
            "checkoutRoomNumber"
        ] = $this->Booking_m->get_h_checkout_today_group_nmNumber(
            $this->session->userdata("idBusiness")
        );
        $data[
            "checkinRoomNumber"
        ] = $this->Booking_m->get_h_checkin_today_group_nmNumber(
            $this->session->userdata("idBusiness")
        );
        $data["new_invoice_number"] = $this->Kamar_m->generate_invoice_number();
        $data["availableRoom"] = $this->Kamar_m->getfile_available_kamar(
